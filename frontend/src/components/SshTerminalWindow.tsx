...
function resolveWebSocketBase() {
  ...
  const wsBaseFromEnv = (import.meta.env.VITE_WS_BASE as string | undefined)?.trim();
  if (wsBaseFromEnv) {
    return wsBaseFromEnv.replace(/^http/i, "ws").replace(/\/+$/, "");
  }
...
export function SshTerminalWindow({
  ...
  const appendOutput = (chunk: string) => {
    setOutput((previous) => sanitizeTerminalText(previous + chunk));
  };
  ...
  const connectSocket = () => {
    ...
    const ws = new WebSocket(`${wsBase}/ws/ssh?cardId=${encodeURIComponent(cardId)}`);
    wsRef.current = ws;
